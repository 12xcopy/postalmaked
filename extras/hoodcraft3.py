...
# Depends on Pillow, and RSPiX.
# using the postal makefile, run 'swig' to get the RSPiX binaries

import PIL.Image, pickle, atexit, shutil, struct, tempfile, os, RSPiX
import io, distutils.spawn, subprocess, PIL.ImageDraw, tkinter
import tkinter.messagebox, tkinter.filedialog, sys, PIL.ImageTk
...
def write_sak(fobj, files):
	...
	pos = 10 + sum(len(fname) + 5 for fname in files.keys())
...
try:
	...
	tkinter.Tk().withdraw()
	tkinter.messagebox.showerror("Hoodcraft", "Could not find homer.sak. Are you sure Hoodcraft is in the POSTAL directory?")
	sys.exit(1)
except invalid_data:
	tkinter.Tk().withdraw()
	tkinter.messagebox.showerror("Hoodcraft", "homer.sak is corrupt.")
	sys.exit(1)
atexit.register(homer.fobj.close)

standard_images = {fname: PIL.Image.open(io.StringIO(homer.read_file("hoods/homer/homer" + fname + ".bmp"))) for fname in [".emptybar", ".emptybarselected", ".fullbar", ".fullbarselected", ".topbar"]}
homer_palette = PIL.Image.open(io.StringIO(homer.read_file("hoods/homer/homer.bmp"))).getpalette()
...
if not gimp_path:
	tkinter.Tk().withdraw()
	tkinter.messagebox.showerror("Hoodcraft", "Could not find GIMP. Make sure it's installed and in the PATH.")
	sys.exit(1)
...
class hood:
	...
	def build(self):
		...
		try:
			...
			for k, v in my_images.items():
				v.save(tmpdir + "/" + k + ".png", "png")
			pimages = list(my_images.keys())
...
pdb.gimp_quit(True)
	""".format(pimages)])
					...
					for gy in range(gh):
						for gx in range(gw):
							...
							for x in range(chunk.width):
								if any(data[p] for p in range(x, len(data), chunk.height)):
									break
								else:
									cleft += 1
							for x in range(chunk.width - 1, -1, -1):
								if any(data[p] for p in range(x, len(data), chunk.height)):
									break
								else:
									cright -= 1
			...
			for k, alpha in homer_alphas.items():
			...
			for y in range(self.background.height):
...
def choose_name():
	...
	new_project_path = localise_path(tkinter.filedialog.asksaveasfilename(
...
def save():
	...
	try:
		...
		tkinter.messagebox.showerror("Hoodcraft", "Failed to save {0}: {1}".format(project_path, ex))
		return False
	modified = False
	update_title()
	return True
...
def open_file():
	...
	if verify_close():
		project_dir, project_file = split_path()
		new_project_path = localise_path(tkinter.filedialog.askopenfilename(
			initialdir = project_dir,
			initialfile = project_file,
			parent = root,
			**file_dialog_options))
		...
		try:
			...
			tkinter.messagebox.showerror("Hoodcraft", "Failed to open {0}: {1}".format(project_path, ex))
			return
...
def build():
	try:
		project.build()
		tkinter.messagebox.showinfo("Hoodcraft", "{0} built successfully.".format(project.name))
	except Exception as ex:
		tkinter.messagebox.showerror("Hoodcraft", "Failed to build {0}: {1}".format(project.name, ex))
...
def browse_bgimage():
	...
	if editor:
		hide_terrain_editor()
	path = localise_path(tkinter.filedialog.askopenfilename(
		parent = root,
	...
	try:
		...
		tkinter.messagebox.showerror("Hoodcraft", "Failed to open {0}: {1}".format(path, ex))
		return
	if project.background:
		if project.background.width != new_background.width or project.background.height != new_background.height:
			tkinter.messagebox.showerror("Hoodcraft", "The new background must be the same size as the existing one - the background is {0}x{1} and this image is {2}x{3}.".format(project.background.width, project.background.height, new_background.width, new_background.height))
			return
...
def browse_alpha(i):
	...
	def ret():
		...
		if not project.background:
			tkinter.messagebox.showerror("Hoodcraft", "You must choose a background image before adding layer images.")
			return
		path = localise_path(tkinter.filedialog.askopenfilename(
			parent = root,
			filetypes = image_filetypes))
		...
		if new_image.width != project.background.width or new_image.height != project.background.height:
			tkinter.messagebox.showerror("Hoodcraft", "Layers must be the same size as the background - the background is {0}x{1} and this image is {2}x{3}.".format(project.background.width, project.background.height, new_image.width, new_image.height))
			return
...
def clear_alpha(i):
	...
	def ret():
		if editor:
			hide_terrain_editor()
		if not project.alpha[i] or tkinter.messagebox.askyesno(
			"Hoodcraft", 
...
def clear_opaque(i):
	...
	def ret():
		if editor:
			hide_terrain_editor()
		if not project.opaque[i] or tkinter.messagebox.askyesno(
			"Hoodcraft", 
...
class terrain_editor(object, tkinter.Frame):
	...
	def delete_selected(self):
		global modified
		if tkinter.messagebox.askyesno("Hoodcraft", "Are you sure you want to delete this region?"):
...
def show_terrain_editor():
	global editor
	if not project.terrain_map:
		tkinter.messagebox.showerror("Hoodcraft", "You must choose a background image before editing the terrain map.")
		return
	if editor:
		editor.focus_force()
		return
