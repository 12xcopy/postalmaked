...
#include "psauce.hpp"
#ifdef PSAUCE_USE_OWN_INPUTEVENTS
int16_t rspGetNextInputEvent(RInputEvent *pie)
{
	return psaucGetNextInputEvent(pie);
}

void rspClearAllInputEvents(void) {psaucClearAllInputEvents();}

// Clears any pending key input events.  Calls rspClearKeyEvents().
void rspClearKeyInputEvents(void)	{psaucClearKeyInputEvents();}

// Clears any pending mouse input events.  Calls rspClearMouseEvents().
void rspClearMouseInputEvents(void);	{psaucClearMouseInputEvents();}

void ClearXInputState() {}

// todo: this.
int16_t GetXInputState(XInputState* xis) {return 0;} 
int16_t GetXInputStateNoUpdate(XInputState* xis) {return 0;}
int16_t GetLastXInputState(XInputState* xis) {return 0;}

int16_t IsXInputButtonPressed() {return 0;}
